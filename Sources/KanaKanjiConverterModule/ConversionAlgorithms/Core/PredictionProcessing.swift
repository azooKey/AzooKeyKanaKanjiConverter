...
extension Kana2Kanji {
    ...
    func getPredictionCandidates(prepart: Candidate, N_best: Int, state: DicdataStoreState? = nil) -> [PostCompositionPredictionCandidate] {
        ...
        while count <= min(prepart.data.count, 3), let element = prefixCandidateData.popLast() {
            ...
            let dicdata = self.dicdataStore.getPredictionLOUDSDicdata(key: totalRuby, state: state).filter {
