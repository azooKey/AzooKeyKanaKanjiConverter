...
英字readingのエントリも利用できます。例えばreadingと表記がともに `GitHub` のエントリは、`Git` の前方一致や `GitHub` の完全一致で英語候補になります。辞書のreading・表記はそのまま保持し、検索時は入力中の小文字を対応する大文字にも一致させます。大文字の入力は大文字だけに一致します。例えば `git` は `GitHub` と `github` の両方に一致し、`Git` は `github` に一致しません。この規則は語中にも適用します。登録語は英字を含むASCII英数字列と、語中の `-`・`'`・`’`・`&`・`.` に対応します。各語末尾のアポストロフィ・ピリオドと、語間の単一半角スペースも許可します（`GPT-4`、`3M`、`don't`、`don’t`、`AT&T`、`U.S.`、`Node.js`、`dogs'`、`New York`、`U.S. Army` など）。英字を含まない語、各語の先頭の記号、単語内の連続する記号、各語末尾の `-`・`&`、アクセント付き文字は対象外です。登録語の先頭・末尾・連続するスペース、タブ・改行・全角スペースは対象外です。入力途中の `gpt-`、`3`、`don'`、`at&`、`u.`、`new ` からの補完は可能で、数字・記号・スペースは完全一致で照合します。`'` と `’` は区別して保持・照合します。OSへの補完問い合わせは従来どおり英字だけの入力に限定します。
...
`anco dict build` の入力は従来の6列TSV（reading・表記・左品詞ID・右品詞ID・意味ID・スコア）です。英字で始まる入力ファイルは、大小文字を区別しないファイルシステムでの衝突を避け、`worddict/[0047].tsv`（G）、`worddict/[0067].tsv`（g）のように先頭文字のコードを4桁の大文字16進数で表します。カタカナ等の既存ファイル名は変更しません。charIDの対応表には末尾に `.`（ID 227）と半角スペース（ID 228）を追加し、既存IDとバイナリ形式は維持します。ピリオド・半角スペースを含むreadingを使うには、新しいビルド結果の `charID.chid` と辞書ファイルを一緒に配布してください。
